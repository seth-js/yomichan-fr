...
class Display extends EventDispatcher {
    ...
    async _setContentTermsOrKanji(type, urlSearchParams, token) {
        ...
        Array.from(document.querySelectorAll('.entry')).forEach((ent) => {
            ent.classList.add('current-dictionary-entry');
      
            const formBoxes = {};
      
            Array.from(
              document.querySelectorAll('.current-dictionary-entry .inflection'),
            ).forEach((inflect) => {
              const [targetPOS] = inflect.textContent.split(' ');
      
              let inflectText = inflect.textContent.split(' ');
              inflectText.shift();
            ...
            Object.entries(formBoxes).forEach((box) => {
              const [pos, info] = box;
              const { inflections, isAutomated, pointerText } = info;
      
              let matchingTag;
      
              Array.from(
                document.querySelectorAll(
                  '.current-dictionary-entry .definition-tag-list',
                ),
              ).forEach((tag) => {
                if (tag.outerHTML.includes(`class="tag" title="${pos}"`)) {
                  matchingTag = tag;
                }
              });
      
              if (matchingTag) {
                let formInfoBox = `<div class="form-info-box">`;
      
                if (isAutomated)
                  formInfoBox += `<div class="automated-result-text">(automated results)</div>`;
      
                formInfoBox += `<div class="pointer-text">${pointerText}</div>`;
      
                formInfoBox += '<ol>';
      
                inflections.forEach((inflection) => {
                  formInfoBox += `<li>${inflection}</li>`;
                });
                ...
                matchingTag.outerHTML += `<span class="show-info-btn">ⓘ</span>${formInfoBox}`;
      
                const recentInfoBtn = Array.from(
                  document.querySelectorAll(
                    '.current-dictionary-entry .show-info-btn',
                  ),
                ).pop();
      
                const recentInfoBox = Array.from(
                  document.querySelectorAll(
                    '.current-dictionary-entry .form-info-box',
                  ),
                ).pop();
      
                recentInfoBtn.addEventListener('mouseenter', (e) => {
                  recentInfoBox.style.display = 'block';
                });
      
                document
                  .querySelector('.current-dictionary-entry')
                  .addEventListener('mouseleave', (e) => {
                    recentInfoBox.style.display = 'none';
                  });
              }
            });

            ent.classList.remove('current-dictionary-entry');
        });
